...
with c_map:
    ...
    if map_col in map_data.columns and map_data[map_col].sum() > 0:
        
        # 1. Define Tooltips (What shows up when you hover)
        tooltip_cols = ["TANC Local"]
        # Add other useful columns if they exist
        for col in ["Total", "Black", "White", "Asian", "Rent Burden"]:
            if col in map_data.columns:
                tooltip_cols.append(col)

        # 2. Render the Map (Heatmap Style)
        m = map_data.explore(
            column=map_col, 
            cmap="YlOrRd",           # Yellow -> Orange -> Red (High contrast)
            scheme="quantiles",      # Forces colors to be distinct (no more washing out)
            k=5,                     # 5 distinct color levels
            tiles="CartoDB positron",
            tooltip=tooltip_cols,    # Clean hover info
            popup=False,             # No clicking needed
            legend_kwds={"caption": "Population Intensity"},
            style_kwds={
                "fillOpacity": 0.7, 
                "weight": 0.5,       # Thinner border lines
                "color": "black"
            } 
        )
        ...
        # Fallback if data is missing or zero (prevents the crash)
        st.warning(f"No data available for '{map_col}' in this local.")
        # Show a plain grey map so it doesn't look broken
        m = map_data.explore(color="lightgrey", tiles="CartoDB positron", style_kwds={"weight": 1})
        st_folium(m, use_container_width=True, height=500)
