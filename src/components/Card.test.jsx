...
describe('Card — non-pinned', () => {
  it('renders local name and tract id', () => {
    ...
    render(<Card feature={f} metric="rent_burden" isPinned={false} onClick={() => {}} />);
  ...
  it('passes the full feature (with geometry) to onClick — regression test', () => {
    ...
    const onClick = vi.fn();
    render(<Card feature={f} metric="rent_burden" isPinned={false} onClick={onClick} />);
  ...
  it('does not crash when given an unknown metric key', () => {
    ...
    expect(() =>
      render(<Card feature={f} metric="unknown_metric" isPinned={false} onClick={() => {}} />)
    ).not.toThrow();
  });
});

describe('Card — pinned', () => {
  it('renders Sheet button and × close button', () => {
    const f = mkFeature({ id: '1', tanc_local: 'X', rent_burden: 50 });
    const onFactSheet = vi.fn();
    render(<Card feature={f} metric="rent_burden" isPinned={true} onClick={() => {}} onFactSheet={onFactSheet} />);
    expect(screen.getByText('Sheet')).toBeTruthy();
    expect(screen.getByText('×')).toBeTruthy();
  });

  it('clicking Sheet calls onFactSheet (and stops propagation)', () => {
    const f = mkFeature({ id: '1', tanc_local: 'X', rent_burden: 50 });
    const onClick = vi.fn();
    const onFactSheet = vi.fn();
    render(<Card feature={f} metric="rent_burden" isPinned={true} onClick={onClick} onFactSheet={onFactSheet} />);
